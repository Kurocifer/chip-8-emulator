...
void TDisplaySDL::init()
{
    ...
    // Create a window
    m_window = SDL_CreateWindow("Kurocifer chip8", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1280, 720, SDL_WINDOW_SHOWN);

    if (!m_window)
    {
        std::string erroWindow(SDL_GetError());
        m_logger->log("Window creation Error" + erroWindow, ELogLevel::ERROR);
        exit(1);
    }
